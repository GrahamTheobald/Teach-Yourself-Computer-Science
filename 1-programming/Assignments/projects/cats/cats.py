...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    candidates = [i for i in paragraphs if select(i)]
...
def about(topic):
    ...
    def select(paragraph):
        for t in topic:
            if t in split(lower(remove_punctuation(paragraph))):
                return True
        return False
    return select
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    indexes = min(len(typed_words), len(source_words))
    correct = [typed_words[i]
               for i in range(indexes) if typed_words[i] == source_words[i]]
    if len(typed_words) == 0:
        ...
        return len(correct) / len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    best_word = min(
        word_list, key=lambda x: diff_function(typed_word, x, limit))

    if typed_word in word_list:
        return typed_word
    elif diff_function(typed_word, best_word, limit) > limit:
        return typed_word
    else:
        return best_word
    # END PROBLEM 5
...
def time_per_word(words, times_per_player):
    ...
    for times in times_per_player:
        times_per_word += [[times[i] - times[i-1]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    list = [[] for p in player_indices]
    for w in word_indices:
        fastest = [time(match, 0, w), 0]
        for p in player_indices:
            if time(match, p, w) < fastest[0]:
                fastest = [time(match, p, w), p]
        list[fastest[1]] += [get_word(match, w)]
    return list
