...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            if place.bees and self.lower_bound <= bounds <= self.upper_bound:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if not gamestate.queen_constructed:
            queen = super().construct(gamestate)
            if queen:
                gamestate.queen_constructed = True
            return queen
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not place.is_hive:
            if place.ant:
                if place.ant.is_container and place.ant.ant_contained:
                    obj[place.ant.ant_contained] = steps
                elif place.ant is not self:
                    obj[place.ant] = steps
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return max(0, self.damage - ((0.25 * distance) + (self.insects_shot / 16)))
        # END Problem Optional 2
