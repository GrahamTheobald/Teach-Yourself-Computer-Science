...
class VendingMachine:
    ...
    def __init__(self, item, cost, stock=0, funds=0):
        self.item = item
        self.cost = cost
        self.stock = stock
        self.funds = funds
    ...
    def vend(self):
        if self.stock == 0:
            str = 'Nothing left to vend. Please restock.'
            if self.funds > 0:
                str += f'Here is your ${self.funds}.'
                self.funds = 0
        elif self.funds < self.cost:
            str = f'Please update your balance with ${self.cost - self.funds} more funds.'
        else:
            str = f'Here is your {self.item}'
            change = self.funds - self.cost
            if change > 0:
                str += f' and ${change} change.'
            else:
                str += '.'
            self.funds = 0
            self.stock -= 1
