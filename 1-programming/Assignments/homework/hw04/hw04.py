...
def balanced(m):
    ...
    if is_planet(m):
        return True
    left_, right_ = left(m), right(m)
    if length(left_) * total_weight(end(left_)) != length(right_) * total_weight(end(right_)):
        return False
    return balanced(end(left_)) and balanced(end(right_))
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert (1 / upper_bound(y)) < (1 / lower_bound(y)), 'My assertion.'
